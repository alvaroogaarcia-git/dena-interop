...
resource "keycloak_openid_client" "piloto_react_frontend" {
...
resource "keycloak_openid_client" "piloto_apisix_gateway" {
  realm_id                     = keycloak_realm.piloto.id
  client_id                    = "apisix-gateway"
  name                         = "APISIX Gateway"
  enabled                      = true
...
resource "keycloak_role" "piloto_writer" {
  realm_id    = keycloak_realm.piloto.id
  name        = "dena-writer"
  description = "Envio de solicitudes de interoperabilidad DENA."
}

resource "keycloak_role" "piloto_admin" {
  realm_id        = keycloak_realm.piloto.id
  name            = "dena-admin"
  description     = "Administracion funcional de la API DENA."
  composite_roles = [keycloak_role.piloto_reader.id, keycloak_role.piloto_writer.id]
}

resource "keycloak_user" "piloto_testuser" {
  realm_id       = keycloak_realm.piloto.id
  username       = "testuser"
  enabled        = true
  email          = "testuser@dena.local"
...
resource "keycloak_user_roles" "piloto_testuser" {
  realm_id = keycloak_realm.piloto.id
