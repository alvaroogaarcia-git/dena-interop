...
STAGING_DB_HOST="${STAGING_DB_HOST:-postgresql-datalake.datalake.svc.cluster.local}"
...
find_controller_service_id() {
  local name="$1"
  curl --http1.1 -skf --max-time 120 -H "Authorization: Bearer $TOKEN" \
...
staging_dbcp_id="$(find_controller_service_id "Datalake DBCP" || true)"
if [[ -z "$staging_dbcp_id" ]]; then
  staging_dbcp_id="$(
    post_json "$NIFI_BASE_URL/nifi-api/process-groups/$group_id/controller-services" "$(create_controller_service_payload \
      "Datalake DBCP" \
      "org.apache.nifi.dbcp.DBCPConnectionPool" \
      "{\"Database Connection URL\":\"jdbc:postgresql://$STAGING_DB_HOST:$STAGING_DB_PORT/$STAGING_DB_NAME\",\"Database Driver Class Name\":\"org.postgresql.Driver\",\"Database Driver Locations\":\"$JDBC_JAR\",\"Database User\":\"$DB_USER\",\"Password\":\"$STAGING_DB_PASSWORD\"}")" | json_get_id
  )"
fi
reader_id="$(find_controller_service_id "JSON Record Reader" || true)"
if [[ -z "$reader_id" ]]; then
  reader_id="$(
    post_json "$NIFI_BASE_URL/nifi-api/process-groups/$group_id/controller-services" "$(create_controller_service_payload \
      "JSON Record Reader" \
...
enable_service() {
  ...
  entity="$(curl --http1.1 -skf --max-time 120 -H "Authorization: Bearer $TOKEN" \
...
staging_id="$(find_processor_id "Persist Staging Batch" || true)"
...
promote_id="$(find_processor_id "Promote Staging To Main" || true)"
...
configure_processor() {
  ...
  local scheduling_period="${4:-}"
  local version

  version="$(curl --http1.1 -skf --max-time 120 -H "Authorization: Bearer $TOKEN" \
    "$NIFI_BASE_URL/nifi-api/processors/$processor_id" | \
    python3 -c 'import json,sys; print(json.load(sys.stdin)["revision"]["version"])')"
  if [[ -n "$scheduling_period" ]]; then
    put_json "$NIFI_BASE_URL/nifi-api/processors/$processor_id" \
      "{\"revision\":{\"version\":$version},\"component\":{\"id\":\"$processor_id\",\"config\":{\"properties\":$properties,\"autoTerminatedRelationships\":$auto_terminated,\"schedulingStrategy\":\"TIMER_DRIVEN\",\"schedulingPeriod\":\"$scheduling_period\"}}}" >/dev/null
    return
  fi
  put_json "$NIFI_BASE_URL/nifi-api/processors/$processor_id" \
...
stop_processor "$staging_id"
stop_processor "$promote_id"
...
configure_controller_service "$staging_dbcp_id" \
  "{\"Database Connection URL\":\"jdbc:postgresql://$STAGING_DB_HOST:$STAGING_DB_PORT/$STAGING_DB_NAME\",\"Database Driver Class Name\":\"org.postgresql.Driver\",\"Database Driver Locations\":\"$JDBC_JAR\",\"Database User\":\"$DB_USER\",\"Password\":\"$STAGING_DB_PASSWORD\"}"

echo "Habilitando servicios de controlador"
enable_service "$dbcp_id"
enable_service "$staging_dbcp_id"
enable_service "$writer_id"
enable_service "$reader_id"
...
connect_processors() {
  ...
  if curl --http1.1 -skf --max-time 120 -H "Authorization: Bearer $TOKEN" \
    "$NIFI_BASE_URL/nifi-api/flow/process-groups/$group_id" | \
    python3 -c 'import json,sys
...
connect_processors "$query_id" "Query Verticales Incremental" "$staging_id" "Persist Staging Batch" success
connect_processors "$staging_id" "Persist Staging Batch" "$promote_id" "Promote Staging To Main" success
...
start_processor "$staging_id"
start_processor "$promote_id"
